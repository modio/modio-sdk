...
{
	class GetUserFollowingOp
	{
	public:
		GetUserFollowingOp(Modio::UserID UserID) : ID(UserID) {}

		template<typename CoroType>
		void operator()(CoroType& Self, Modio::ErrorCode ec = {})
		{
			MODIO_PROFILE_SCOPE(GetUserFollowing);
			reenter(CoroutineState)
			{
				do
				{
					yield Modio::Detail::PerformRequestAndGetResponseAsync(
						ResponseBodyBuffer,
						Modio::Detail::GetUserFollowingRequest.SetUserID(ID).AddLimitQueryParam()
	...
	template<typename GetUserFollowingCompleteCallback>
	void GetUserFollowingAsync(Modio::UserID UserID, GetUserFollowingCompleteCallback&& OnComplete)
	{
		return ModioAsio::async_compose<GetUserFollowingCompleteCallback,
										void(Modio::ErrorCode, Modio::Optional<Modio::UserList>)>(
			Modio::Detail::GetUserFollowingOp(UserID), OnComplete,
