...
#include <asio/yield.hpp>
		class AuthenticateUserDelegatedTokenOp
		{
		public:
			AuthenticateUserDelegatedTokenOp(Modio::Detail::HttpRequestParams AuthenticationParams)
			{
			...
			void operator()(CoroType& Self, Modio::ErrorCode ec = {})
			{
				...
				reenter(LocalState->CoroutineState)
				{
						...
						if (Error.has_value())
						{
								...
								ErrRef == Modio::ApiError::XBoxLiveNotAllowedToInteractWithUGC)
							{
								Self.complete(Modio::make_error_code(
									Modio::ParentalControlRestriction::ParentalControlRestriction));
								return;
							}
						}
					}
					...
					{
						Modio::Detail::SDKSessionData::UpdateTokenForExistingUser(
							Modio::OAuthToken(LocalState->AuthResponse));
					}
		...
		auto AuthenticateUserDelegatedTokenAsync(Modio::AuthenticationParams User, AuthDoneCallback&& OnAuthComplete)
		{
			Modio::Detail::HttpRequestParams Params =
				Modio::Detail::AuthenticateViaUdtRequest.AddHeaderRaw("X-Modio-Delegation-Token", User.AuthToken)
			...
			return ModioAsio::async_compose<AuthDoneCallback, void(Modio::ErrorCode)>(
				Modio::Detail::AuthenticateUserDelegatedTokenOp(Params), OnAuthComplete,
