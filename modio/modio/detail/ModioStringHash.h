...
namespace Modio
{
	namespace Detail
	{
		...
		inline constexpr std::uint32_t hash_32_fnv1a_const(const char* const str,
														   const std::uint32_t value = val_32_const) noexcept
		{
			return (str[0] == '\0') ? value : hash_32_fnv1a_const(&str[1], (value ^ uint32_t(str[0])) * prime_32_const);
		}

		inline constexpr std::uint64_t hash_64_fnv1a_const(const char* const str,
