...
namespace Modio
{
	...
	struct OAuthToken
	{
		...
		OAuthToken(const std::string& InToken, Modio::Timestamp InExpireDate)
			: Token(InToken),
				...
				State(Modio::OAuthTokenState::Valid)
		{}

		/// @docinternal
		/// @brief Constructor that creates token from an access token object
		OAuthToken(Modio::AccessTokenObject AccessToken)
		...
		Modio::OAuthTokenState GetTokenState() const
		{
			if (State == Modio::OAuthTokenState::Valid)
			{
				auto Now = std::chrono::system_clock::now();
				std::size_t CurrentUTCTime =
					std::size_t(std::chrono::duration_cast<std::chrono::seconds>(Now.time_since_epoch()).count());

				return CurrentUTCTime <= ExpireDate ? Modio::OAuthTokenState::Valid : Modio::OAuthTokenState::Invalid;
			}
			return Modio::OAuthTokenState::Invalid;
		}
		...
		operator bool() const
		{
			return GetTokenState() == Modio::OAuthTokenState::Valid;
		}
		...
		{
			if (GetTokenState() == Modio::OAuthTokenState::Valid)
			{
				return Token.value();
			}
			return {};
		...
		Modio::OAuthTokenState GetRawState() const
		{
			return State;
		}
	private:
	...
	private:
		...
		Modio::OAuthTokenState State = Modio::OAuthTokenState::Invalid;

		/// @docnone
		friend bool operator==(const Modio::OAuthToken& A, const Modio::OAuthToken& B)
		{
	...
	namespace Detail
	{
		typedef Modio::AccessTokenObject AccessTokenObject;
		typedef Modio::OAuthTokenState OAuthTokenState;
		typedef Modio::OAuthToken OAuthToken;
	} // namespace Detail
} // namespace Modio
