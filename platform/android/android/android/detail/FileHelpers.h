...
namespace Modio
{
	namespace Detail
	{
		...
		inline Modio::ErrorCode TranslateFilesystemError(int AndroidError,
														 Modio::Detail::FilesystemErrorContext Context)
		{
			if (AndroidError == 0)
			{
				return {};
			...
			Modio::Detail::Logger().Log(Modio::LogLevel::Error, Modio::LogCategory::File,
										"Raw Android filesystem error {} received", AndroidError);
			switch (AndroidError)
			{
				case ENOENT:
