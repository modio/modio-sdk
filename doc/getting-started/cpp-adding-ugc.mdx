...
To submit UGC, first create a mod handle using [`GetModCreationHandle`](/cppsdk/refdocs/#getmodcreationhandle) and use that handle when calling [`SubmitNewModAsync`](/cppsdk/refdocs/#submitnewmodasync). Note that the newly created mod will remain hidden until a mod file is added in the next step.
...
UGC details can be edited in-game using [`SubmitModChangesAsync`](/cppsdk/refdocs/#submitmodchangesasync).  This function allows you to edit multiple parameters with a single call.  It takes a `Modio::ModID` of the UGC to edit, a `Modio::EditModParams` containing one or more parameters to be altered, and a callback that will contain an optional updated `Modio::ModInfo` object on success.

Note that updating the mod file itself is done via [`SubmitNewModFileForMod`](/cppsdk/refdocs/#submitnewmodfileformod), as detailed above in [`Submitting UGC data`](#submitting-ugc-data).
...
Congratulations! You have completed all the [C++ SDK Getting Started Guides](/cppsdk/#c-sdk-core-setup-guides)! Your game should now be equipped with mod.io's core UGC functionally.

The time's come to customize your game by exploring our [Features](/features) section. Here you can select features that suit your game's requirements.