...
custom_edit_url: https://github.com/modio/modio-sdk-internal/blob/develop/doc/getting-started/cpp-structure.mdx
---
...
Every async callback takes an [ErrorCode](https://docs.mod.io/cppsdk/refdocs#ErrorCode) as its first parameter, with any results wrapped in [Optional](https://docs.mod.io/cppsdk/refdocs#Optional) to check if a result is valid or empty.
...
:::note
Even if the SDK shuts down while asynchronous operations are pending, the remaining callbacks will still execute **exactly once**. In this case, the callback receives an [ErrorCode](https://docs.mod.io/cppsdk/refdocs#ErrorCode) to indicate a canceled state. Your project should handle gracefully this behavior as part of your error handling.
:::

### Maintaining the SDK event loop

In order to provide a non-blocking implementation, the SDK operates an internal event loop. This event loop only runs on the thread which calls [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers).

The event loop, all internal event handlers and callbacks provided to the mod.io SDK execute on the thread invoking [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers). [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers) must only be called on one thread, otherwise, its behavior is undefined.

:::note
If you stop calling [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers), any pending asynchronous API methods will not complete and their associated callbacks will not be invoked. It also includes the internal data allocated for those operations, as well as the release of any allocated objects.
:::

## Thread safety

Given that [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers) performs all the work of the SDK and executes the callbacks that you provide as handlers for the completion of async functions, your application needs to be calling it at regular intervals. However, you may not wish to do so on the main thread of your application, given that the function has to execute for long enough to actually 'get some work done'.

The mod.io SDK supports the execution of [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers) on a background thread while your application invokes other SDK functions on the main thread, for example in response to user input in your application's user interface. 

Whilst it is safe to call all other SDK functions from a different thread to the one executing [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers), it is important to note that our existing guarantees are maintained, namely, you'll still receive exactly one callback invocation per asynchronous function you run, and callbacks you provide to those methods will be executed on the thread running [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers). 

:::note
[RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers) should still only be called on a single thread - it is not safe to call [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers) from multiple threads, either simultaneously or sequentially.

By using a background thread for [RunPendingHandlers](https://docs.mod.io/cppsdk/refdocs#runpendinghandlers), you can decouple the frequency with which you perform SDK 'work' from the frequency of your application's main loop for greater performance. 
...
To change a Local Profile's authenticated user, call [`ClearUserDataAsync`](https://docs.mod.io/cppsdk/refdocs#clearuserdataasync) to remove the authenticated user, and then re-authenticate as normal.

:::note
A call to [`ClearUserDataAsync`](https://docs.mod.io/cppsdk/refdocs#clearuserdataasync) removes the authenticated user from the local device, and disables UGC management. Any installed content is marked for uninstallation from local storage if no other Local Profiles contain authenticated users with active subscriptions to it.
:::

To add a newly authenticated user or switch to one already-authenticated without removing the current one, swap to another Local Profile by calling [`ShutdownAsync`](https://docs.mod.io/cppsdk/refdocs#shutdownasync), then re-initialize via [InitializeAsync](https://docs.mod.io/cppsdk/refdocs#initializeasync) specifying a different Local Profile name in the initialization parameters you supply.

## Error handling

Callback functions in the SDK either return a value or provide an [ErrorCode](https://docs.mod.io/cppsdk/refdocs#ErrorCode) value. It is a numeric error code with a category and an associated string message.
...
This is where the second overload of [ErrorCodeMatches](https://docs.mod.io/cppsdk/refdocs#ErrorCodeMatches) comes in. It allows you to query if the error satisfies a particular condition, such as 'does this code represent some kind of networking error':
...
Now you know more about how the plugin works, it's time to set up [User Authentication](https://docs.mod.io/cppsdk/user-authentication) or skip straight to [Searching for UGC](https://docs.mod.io/cppsdk/searching-ugc).
