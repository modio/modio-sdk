...
##### Dynamic library mode (unsupported)

The SDK can optionally be built as a dynamic/shared library (`modioShared`) that exports the C++ API. This is **not** a supported integration path and is **disabled by default** - header-only, separate compilation, and static library modes remain the recommended ways to consume the SDK.

:::warning
The dynamic library carries **no API or ABI stability guarantees** whatsoever. The exported interface is the C++ API, which is sensitive to the compiler, compiler version, standard library, and build settings used. A binary built from one SDK version (or toolchain) is not guaranteed to be compatible with any other. Use at your own risk.
:::

To opt in, pass `-DMODIO_BUILD_DYNAMIC_LIBRARY=ON` at configure time, then build the `modioShared` target explicitly (it is excluded from the default build):

```zsh
cmake -S <modio-sdk folder> --preset=win -DMODIO_BUILD_DYNAMIC_LIBRARY=ON
cmake --build <modio-sdk folder>/out/build/win --target modioShared
```
