...
```
<Persistent Storage>/mod.io/<Game ID>/<String User ID #1>/<Cached Auth and Profile data for platform user #1>
<Persistent Storage>/mod.io/<Game ID>/<String User ID #2>/<Cached Auth and Profile data for platform user #2>
```

When your game starts, you can detect the user associated with the current controller and pass in the stable string representation of their platform ID as the SessionID. If the user has previously authenticated with mod.io for this game on this device, their authentication status would be maintained.

In the case of a PC title with user-provided profile names, the folder structure would be more like the following:

```
%USERDATA%/mod.io/<Game ID>/MyProfile1/<Cached Auth and Profile data for mod.io account #1>
...
* [Monetization - Virtual Currency](#monetization-virtual-currency)
* [Monetization - Real Currency](#monetization-real-currency)
...
| --- | --- |
| `PendingOnlyResults` | Only include mods in search results that are pending moderation. For moderation and testing purposes. Set to "true" to enable. Warning: Should only be enabled when the user is a moderator or admin of the game, setting in shipping builds is not recommended. |
| `PlatformOverride` | Set the platform to be used when making requests to show mods for that platform instead. For moderation and testing purposes. |
...
The mod.io multiplayer framework is primarily focused on facilitating the initialization and authentication of dedicated servers with the mod.io backend. This allows authentication to occur in a headless environment; without human intervention.

To do this a server operator must create a Server OAuth token on their [mod.io API Settings page](https://mod.io/me/access). In addition to this OAuth token, a server operator will need a list of UGC they want their Server to use, in the form of a list of Mod IDs. How these pieces of information are handed to the mod.io SDK is up to the developer, however they are needed at the time of initialization of the SDK.

A general overview of the framework can be found [here](/cppsdk/multiplayer/), however in brief the dedicated server framework can be thought of as a seperate method of initialization for the mod.io service, with some additional helpers for handling UGC in use by clients. While it does not directly handle any kind of communication or replication between the Server and Clients, it exposes basic functionality to help get you started with creating multiplayer UGC games.

A Server is expected to have its own list of UGC it uses that all joining Clients are expected to download. Similarly, all joining Clients can have their own lists (subscriptions) of UGC that they use, which can be registered with the Server so that it can maintain a persistent list of UGC Clients are using. This list can then be shared to all Clients, along with the Server's own UGC, to be initalized on every client as a [Temporary Mod Set](#temporary-mod-sets).

#### Initialization

Initialization follows much the same process as the standard mod.io initialization process, with a few added parameters.

```cpp
void InitializeModioServer()
{
...
:::note
While the `ApiKey` and `GameId` are static, the Server `Token`, `ModDirectory`, and `Mods` list are not (as they will differ between servers), and we recommend you make these configurable in some way, be it via a config file, launch arguments, or other.

`Mods` can be empty (for an un-modded Server) and additional UGC can be requested as part of the UGC Acquisition process below.
:::

#### UGC Acquisition

To have the Server download the UGC indicated in `Modio::ServerInitializeOptions::Mods` you will need to call `InstallOrUpdateServerModsAsync` manually to download the mods onto your server. This requires that mod management be active prior to calling.

```cpp
void InstallServerMods(std::vector<Modio::ModID> AdditionalMods)
{
...
:::note
As part of the download process, all ModIDs are checked against the OAuth token given during initialization for the purposes of [Monetization](#monetization), and any UGC that the user who created the OAuth token does not have ownership of will fail to download.

You can pass an empty vector of ModIDs to this function and it will only download the ModIDs indicated during Init. Otherwise any mods listed in `AdditionalMods` above will be installed in addition to the Mods given during initialization.
:::

#### Querying Server UGC

If you need to query what UGC the Server has downloaded, you can do it via `Modio::QuerySystemInstallations` to return the full list of installed UGC.

```cpp
std::map<Modio::ModID, Modio::ModCollectionEntry> ServerMods = Modio::QuerySystemInstallations();
```

#### Joining Client UGC

While the Mod.io Server framework does not perform any network communication or replication itself, it does provide functionality to simplify the process of managing client-side UGC.

When a client joins the Server/Game, it can send a list of `Modio::ModID`s to your server instance. The server then can then call `Modio::RegisterClientModsWithServerAsync` which will internally track the full list of all Client UGC ever passed to it, returning the full list for you to filter/approve based on your own game-specific logic. Your server can then transmit any new mods back to all connected Clients to ensure all UGC is properly propagated.

:::note
This code should only be called on the Server!
:::

```cpp
void OnClientModListReceived(const std::vector<Modio::ModID>& JoiningClientsModList)
{
...
:::note
While our Server framework maintains a list of the UGC registered with it, it does not automatically download the given UGC for you. It only confirms that the ID given is valid and then returns the up to date list.

We do not recommend that you install Client UGC registered in this way on the Server, as this constitutes a substantial security risk where clients can install and execute arbitrary files on your Server machines.

Should you wish to install Client UGC on the Server, the best place to do so is when `RegisterClientModsWithServerAsync` returns the full list of registered UGC. At this point, you can optionally filter, then initialize the UGC into a [Temporary Mod Set](#temporary-mod-sets).
:::

#### Querying Client UGC

In the event that you need to query what UGC is currently registered with the Server you can do so by calling `Modio::GetRegisteredClientMods()`.

```cpp
std::set<Modio::ModID> RegisteredMods = Modio::GetRegisteredClientMods();
```

#### Clearing Client UGC

While we do not provide a method of removing any given UGC from the registered list directly, you can clear the list entirely by calling `Modio::ClearRegisteredClientMods()`

The reasoning behind this is that it is possible that a modded item or feature is still in use on a server after the client who brought it leaves. Additionally, as the server only tracks the IDs of registered client UGC there is no performance concern on the server, and when a Client leaves they should close their Temp Mod Set which will free up their own resources.

The purpose of `ClearRegisteredClientMods` is for the closing or refreshing of a server, or when all players have exited, in some cases.

#### Listen Servers

The Mod.io Server framework is not intended for use in Listen Servers (or P2P multiplayer). In these cases it is expected that [Temporary Mod Sets](#temporary-mod-sets) will be leveraged for all clients and the host.
...
As mentioned in [the quick-start section on initialization](#initialization-and-teardown), the SDK supports [RunPendingHandlers](/cppsdk/refdocs/#runpendinghandlers) being run on a secondary thread that already exists or a dedicated background thread you create specifically to perform SDK work.
...
mod.io provides two different options for supporting monetization within your game - Virtual Currency, and Real Currency. You can toggle between these options on your game's monetization dashboard, but you can only ship with one of them enabled and used in your title.
...
You can call `<<PurchaseModAsync>>` to purchase a given mod. PurchaseModAsync takes two parameters = the ModID of the mod to purchase, and the ExpectedVirtualCurrencyPrice, which is the price displayed to the user from `<<ListAllModsAsync>>`. You must include this parameter for safety, so the user is not charged more or less than the price displayed to them in case the price of the mod has changed between the call to ListAllModsAsync and purchase time.
...
:::note
There are different monetization models available for games using mod.io - Virtual Currency, and Real Currency. You can toggle between these options during development in your game's monetization dashboard, but you can only ship with one option enabled. For more information about the difference between these two options, head to the Monetization Getting Started guide.
:::

The mod.io SDK supports all of the mod.io monetization features, allowing you to sell mods via USD Marketplace to your players, with a share of the revenue split between creators and your studio. Visit [mod.io monetization system](/monetization/) for an overview.
...
The mod.io monetization features are enabled as part of the onboarding process on your game profile. Once that is setup, ensure that you have enabled USD Marketplace in your game's profile. Once that is done, there is nothing further you need to do for initialization in the SDK.

Ensure that you have set the appropriate Portal when initializing the SDK for the portal you are using for purchasing - for instance, on Steam, you must initialize with Modio::Portal::Steam in order to perform appropriate platform SKU lookups.
